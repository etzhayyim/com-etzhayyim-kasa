(ns kasa.methods.social
  "social.cljc — 嵩 (かさ) DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from kasa's own
...
(defn draft-observation-post
  "A dry-run observation post over kasa's own domain (Datafies, from PUBLIC information, the WORLDWIDE annual MAGNITUDE and GROWTH of computing capacity across four domains: STORAGE (HDD + SSD…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "kasa R0: live social posting is Council Lv6+ + operator + "
